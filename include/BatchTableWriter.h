...
private:
    ...
    void insertRecursive(std::vector<ConstantSP>* row, DestTable* destTable, int colIndex, T first, Targs... Fargs){
        ConstantSP tmp =  createObject(destTable->colDefsTypeInt->getInt(colIndex), first);
        if(int(tmp->getType()) != destTable->colDefsTypeInt->getInt(colIndex))
            throw RuntimeException("Failed to insert data, the type of argument does not match the type of column.");
        row->push_back(tmp);
        colIndex++;
